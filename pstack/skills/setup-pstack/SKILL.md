...
Write `~/.config/opencode/pstack-models.md`, an always-applied rule that sets pstack's model per role.
...
**(a) Ask for a budget.** Prefer question over free text. Offer these four options with these exact labels, and name the current budget when the rule records one.
...
Write `~/.config/opencode/pstack-models.md` with `alwaysApply: true`, a `# budget` line with the chosen label and its target effort, and one line per role, using the same labels poteto-mode uses. Overwrite the whole file so re-runs stay idempotent. Shape:
